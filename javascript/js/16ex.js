 //오늘 시간표 - time, todo 배열 선언
        ...
        const timeTr2 = document.getElementById('time')
        const todoTr2 = document.getElementById('todo')

        //데이터만 다르고 실행 내용은 같음 -> 함수로 리팩토링
        function timeline(timeArr,todoArr,title){
                            //형식 매개변수 (값은 실행할 때 전달 되어 저장)
            timeTr.innerHTML = '<th>TIME</th>' //버튼 누르면 안에 지워라
            todoTr.innerHTML = '<th>TODO</th>'
            title.textContent = 'title + 시간표'
            for(let t of timeArr){
                const td = document.createElement('td')
                td.textContent = t
                timeTr.appendChild(td) //안에 채워라
            }
        ...
        tom.addEventListener('click', ()=>{
            timeline(time, todo, '오늘') //위에서 선언한 배열 이름 함수로 전달
        })
        to.addEventListener('click', function(){
            timeline(time, todo, '내일')
        })
