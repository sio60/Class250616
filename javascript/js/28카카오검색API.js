...
fetch(url,header)                      //요청 url 주소로 보내기
    .then(response => {                 //요청에 대한 응답 수신, 콜백 함수 인자 response에 저장
        // console.log(response)
        return response.json()          //응답의 body(데이터본문)을 JS 객체로 변환
    })
    .then(data => {      
        result = data               //위의 then에서 리턴한 값을 콜백함수 인자 data에 저장
        console.log('data:',data)
        console.log('데이터 분해 1:', data.documents[0])      
        console.log('데이터 분해 2:', data.documents[0].doc_url)      
        console.log('데이터 분해 2:', data.documents[0].image_url)
        //함수 실행
    ...
    //다음의 console 명령이 실행 됨 (비동기 처리 명령어)
    console.log('응답데이터'. result)  //undefined
